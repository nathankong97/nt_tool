...
def main():
    ...
    for date in formatted_dates:
        result = ua.calendar_search(date, "SFO", "TYO", proxy.proxy_ip)
    ...
    segments = []
    for date in date_keys[:16]:
        result = ua.search2("SFO", "HND", date, "http://scraperapi:7ad9be8666fb0b04f8771e53cd642d67@proxy-server.scraperapi.com:8001")
        flights = process_lowest_award_itinerary(result.json(), fare_class)
        for f in flights:
            for s in f.segments:
                segments.append(s)
        #global_time.sleep(10)
...
if __name__ == "__main__":
    sys.stdout = open(f'/root/pi/nt_tool/united_award_tracking/SFO_{datetime.now()}.log', 'w')
    main()
