...
def main():
    ...
    segments = []
    for date in date_keys[:18]:
        result = ua.search2("JFK", "HND", date, "http://scraperapi:7ad9be8666fb0b04f8771e53cd642d67@proxy-server.scraperapi.com:8001")
        flights = process_lowest_award_itinerary(result.json(), fare_class)
        for f in flights:
            for s in f.segments:
                segments.append(s)
        #global_time.sleep(10)dandaniel
...
if __name__ == "__main__":
    sys.stdout = open(f'/root/pi/nt_tool/united_award_tracking/NYC_{datetime.now()}.log', 'w')
    main()
